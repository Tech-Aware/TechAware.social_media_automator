# Location: tests/infrastructure/utils/test_file_reader.py
...
def test_read_prompt_file():
    """
    Test that read_prompt_file correctly reads and returns file content.
    """
    ...
    with patch('builtins.open', mock_open(read_data=mock_content)) as mock_file:
        with patch('os.path.dirname') as mock_dirname:
            ...
            result = read_prompt_file('test_prompt')
...
def test_read_prompt_file_file_not_found():
    """
    Test that read_prompt_file raises FileNotFoundError when file doesn't exist.
    """
    with pytest.raises(FileNotFoundError):
        with patch('builtins.open', mock_open()) as mock_file:
            mock_file.side_effect = FileNotFoundError()
            read_prompt_file('non_existent_file')

def test_read_prompt_file_io_error():
    """
    Test that read_prompt_file raises IOError when there's an IO error.
    """
    with pytest.raises(IOError):
        with patch('builtins.open', mock_open()) as mock_file:
            mock_file.side_effect = IOError()
            read_prompt_file('error_file')